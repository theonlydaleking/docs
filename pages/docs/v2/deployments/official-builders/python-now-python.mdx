...
The Web Server Gateway Interface (WSGI) is a calling convention for web servers to forward requests to web applications written in the Python. You can use WSGI with frameworks such as [Flask](https://zeit.co/examples/python-flask) or Django.

Instead of defining a `handler`, define an `app`.
...
@app.route('/<path:path>')
def catch_all(path):
    return Response("<h1>Hello from Flask on Now<h1><p>Path: %s</p>" % (path))
```

Most frameworks use their own implementation of routing but you can use a catch-all route and configure [Now Routes](https://zeit.co/docs/v2/deployments/routes) to each lambda.
